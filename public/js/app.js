...
// Navigation pages
const pages = ['overview', 'settings', 'installer', 'monitoring', 'prometheus-config', 'dataprepper-config', 'snmp-query', 'mib-importer', 'oid-library', 'database', 'user-management', 'activity-logs', 'query-explorer', 'debugging', 'system-update', 'backup-db-configs', 'backup-destinations', 'backup-run', 'backup-history', 'network-topology'];
...
function showPage(pageId) {
  ...
  if (pageId === 'overview') {
    ...
    checkForUpdates();
    loadGithubToken();
  } else if (pageId === 'network-topology') {
    pageTitle.textContent = 'Network Topology';
    pageDesc.textContent = 'Interactive network topology map — Source of Truth for all devices.';
    loadTopologyGraph();
...
let topologyNodesData = [];
let topologyEdgesData = [];

const topologyNodeColors = {
  server: '#58a6ff',
  switch: '#10b981',
  router: '#f59e0b',
  firewall: '#ef4444',
...
function initTopologyGraph(nodes, edges) {
  const container = document.getElementById('topology-graph');
  if (!container) return;

  topologyNodesData = nodes.map(n => ({
    id: n.id,
    label: n.name + '\n' + n.ip,
    title: `<b>${n.name}</b><br>IP: ${n.ip}<br>Type: ${n.deviceType}<br>Status: ${n.status}<br>Sources: ${n.sources.join(', ')}`,
    color: {
      background: topologyNodeColors[n.deviceType] || topologyNodeColors.unknown,
      border: topologyStatusColors[n.status] || topologyStatusColors.unknown,
      highlight: { background: '#58a6ff', border: '#79c0ff' }
    },
    shape: 'dot',
    size: n.sources.includes('PROM') || n.sources.includes('KUMA') ? 20 : 16,
    font: { color: '#e6edf3', size: 10, face: 'monospace', strokeWidth: 2, strokeColor: '#0d1117' },
    borderWidth: 3,
    x: n.x,
    y: n.y,
    fixed: n.x !== undefined && n.y !== undefined ? { x: true, y: true } : false,
    _raw: n
  }));

  topologyEdgesData = edges.map(e => ({
  ...
  const visNodes = new vis.DataSet(topologyNodesData);
  const visEdges = new vis.DataSet(topologyEdgesData);
  ...
  const options = {
    ...
    interaction: { hover: true, tooltipDelay: 200, zoomView: true, dragView: true, multiselect: false, navigationButtons: false },
    edges: { smooth: { type: 'continuous' } },
    layout: { improvedLayout: true, hierarchical: false }
  };

  topologyNetwork = new vis.Network(container, { nodes: visNodes, edges: visEdges }, options);

  topologyNetwork.on('dragEnd', async function(params) {
    if (params.nodes.length > 0) {
      for (const nodeId of params.nodes) {
        const pos = topologyNetwork.getPosition(nodeId);
        try {
  ...
  topologyNetwork.on('doubleClick', function(params) {
    if (params.nodes.length > 0) {
      const nodeId = params.nodes[0];
      const node = topologyNodesData.find(n => n.id === nodeId);
      if (node && node._raw) {
        showTopologyNodeDetails(node._raw);
      }
    }
  });

  document.getElementById('topology-node-count').textContent = nodes.length;
  document.getElementById('topology-edge-count').textContent = edges.length;
}

function showTopologyNodeDetails(node) {
  const sources = node.sources.map(s => {
    const sc = topologySourceColors[s] || topologySourceColors.MANUAL;
    return `<span style="display:inline-block;padding:2px 6px;background:${sc.bg};color:${sc.color};border-radius:3px;font-size:10px;font-weight:600;">${s}</span>`;
  }).join(' ');

  const labels = Object.entries(node.labels || {}).map(([k, v]) => `<tr><td style="padding:2px 8px 2px 0;color:var(--text-muted);font-size:11px;">${escapeHtml(k)}</td><td style="padding:2px 0;font-size:11px;color:var(--text-white);">${escapeHtml(String(v))}</td></tr>`).join('');

  showFeedback('info', `${node.name} (${node.ip})`, `
    <div style="text-align:left;font-size:12px;">
      <div style="margin-bottom:6px;"><strong>Status:</strong> <span style="color:${topologyStatusColors[node.status]};">${node.status.toUpperCase()}</span></div>
      <div style="margin-bottom:6px;"><strong>Type:</strong> ${escapeHtml(node.deviceType)}</div>
      <div style="margin-bottom:6px;"><strong>Sources:</strong> ${sources}</div>
      ${labels ? '<div style="margin-top:8px;"><strong>Labels:</strong><table style="margin-top:4px;">' + labels + '</table></div>' : ''}
      <div style="margin-top:8px;"><button class="btn btn-danger" onclick="topologyDeleteDevice('${escapeAttr(node.id)}')" style="padding:4px 8px;font-size:10px;">Delete Device</button></div>
    </div>
  `);
}
...
async function loadTopologyGraph() {
  try {
    const res = await fetch('/api/v1/topology/graph', {
      headers: { 'Authorization': 'Bearer ' + localStorage.getItem('hephaestus_session_token') }
    });
    const data = await res.json();
    if (!data.success) throw new Error(data.error);
    initTopologyGraph(data.data.nodes || [], data.data.edges || []);
  } catch (e) {
    console.error('[Topology] Failed to load graph:', e.message);
    initTopologyGraph([], []);
  }
}

function topologyAutoScan() {
  document.getElementById('topology-scan-bar').style.display = 'flex';
}

async function topologyRunScan() {
  const ipRange = document.getElementById('topology-ip-range').value.trim();
  const community = document.getElementById('topology-snmp-community').value.trim() || 'public';
  const statusMsg = document.getElementById('topology-status-msg');
  const scanBar = document.getElementById('topology-scan-bar');

  statusMsg.style.display = 'block';
  statusMsg.className = 'alert alert-info';
  statusMsg.textContent = 'Scanning network... This may take a while for large IP ranges.';

  try {
    const res = await fetch('/api/v1/topology/scan', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + localStorage.getItem('hephaestus_session_token')
      },
      body: JSON.stringify({ ipRange: ipRange || undefined, snmpCommunity: community })
    });
    const data = await res.json();
    if (!data.success) throw new Error(data.error);
    ...
    statusMsg.textContent = `Scan complete: ${data.data.meta.totalNodes} nodes, ${data.data.meta.totalEdges} edges found. Sources: ${data.data.meta.sources.join(', ')}`;
    setTimeout(() => { statusMsg.style.display = 'none'; }, 5000);
  } catch (e) {
    statusMsg.className = 'alert alert-danger';
    statusMsg.textContent = 'Scan failed: ' + e.message;
  }
}

function showAddTopologyDeviceModal() {
  const modal = document.getElementById('modal-topology-device');
  if (modal) modal.classList.add('active');
  document.getElementById('topo-device-name').value = '';
  document.getElementById('topo-device-ip').value = '';
  document.getElementById('topo-device-type').value = 'server';
  document.getElementById('topo-device-error').style.display = 'none';
}

async function submitAddTopologyDevice() {
  const name = document.getElementById('topo-device-name').value.trim();
  const ip = document.getElementById('topo-device-ip').value.trim();
  const deviceType = document.getElementById('topo-device-type').value;
  const errorEl = document.getElementById('topo-device-error');

  if (!name || !ip) {
    errorEl.textContent = 'Name and IP are required.';
    errorEl.style.display = 'block';
    return;
  }

  try {
    const res = await fetch('/api/v1/topology/device', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + localStorage.getItem('hephaestus_session_token')
      },
      body: JSON.stringify({ name, ip, deviceType })
    });
    const data = await res.json();
    if (!data.success) throw new Error(data.error);

    closeModal('modal-topology-device');
    await loadTopologyGraph();
    showFeedback('success', 'Device Added', `"${name}" added to topology.`);
  } catch (e) {
    errorEl.textContent = e.message;
    errorEl.style.display = 'block';
  }
...
async function topologyDeleteDevice(deviceId) {
  if (!confirm('Delete this device from the topology?')) return;
  try {
    await fetch('/api/v1/topology/device/' + encodeURIComponent(deviceId), {
      method: 'DELETE',
      headers: { 'Authorization': 'Bearer ' + localStorage.getItem('hephaestus_session_token') }
    });
    await loadTopologyGraph();
    showFeedback('success', 'Deleted', 'Device removed from topology.');
  } catch (e) {
    showFeedback('error', 'Delete Failed', e.message);
  }
}

function showAddTopologyEdgeModal() {
  const sourceSelect = document.getElementById('topo-edge-source');
  const targetSelect = document.getElementById('topo-edge-target');
  sourceSelect.innerHTML = '';
  targetSelect.innerHTML = '';

  for (const node of topologyNodesData) {
    const opt1 = new Option(node.label.split('\n')[0] + ' (' + (node._raw?.ip || '') + ')', node.id);
    const opt2 = new Option(node.label.split('\n')[0] + ' (' + (node._raw?.ip || '') + ')', node.id);
    sourceSelect.add(opt1);
    targetSelect.add(opt2);
  }

  if (topologyNodesData.length === 0) {
    sourceSelect.add(new Option('No devices available', ''));
    targetSelect.add(new Option('No devices available', ''));
  }

  document.getElementById('topo-edge-label').value = '';
  document.getElementById('topo-edge-error').style.display = 'none';
  const modal = document.getElementById('modal-topology-edge');
  if (modal) modal.classList.add('active');
}

async function submitAddTopologyEdge() {
  const source = document.getElementById('topo-edge-source').value;
  const target = document.getElementById('topo-edge-target').value;
  const label = document.getElementById('topo-edge-label').value.trim();
  const errorEl = document.getElementById('topo-edge-error');

  if (!source || !target) {
    errorEl.textContent = 'Source and target are required.';
    errorEl.style.display = 'block';
    return;
  }
  if (source === target) {
    errorEl.textContent = 'Source and target must be different.';
    errorEl.style.display = 'block';
    return;
  }

  try {
    const res = await fetch('/api/v1/topology/edge', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + localStorage.getItem('hephaestus_session_token')
      },
      body: JSON.stringify({ source, target, label: label || undefined })
    });
    const data = await res.json();
    if (!data.success) throw new Error(data.error);

    closeModal('modal-topology-edge');
    await loadTopologyGraph();
    showFeedback('success', 'Connection Added', 'Edge created between devices.');
  } catch (e) {
    errorEl.textContent = e.message;
    errorEl.style.display = 'block';
  }
...
async function topologyClearAll() {
  if (!confirm('Clear ALL topology data? This will remove all devices and connections.')) return;
  try {
    for (const node of topologyNodesData) {
      await fetch('/api/v1/topology/device/' + encodeURIComponent(node.id), {
        method: 'DELETE',
        headers: { 'Authorization': 'Bearer ' + localStorage.getItem('hephaestus_session_token') }
      });
    }
    await loadTopologyGraph();
    showFeedback('success', 'Cleared', 'All topology data removed.');
  } catch (e) {
